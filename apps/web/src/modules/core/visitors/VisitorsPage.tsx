...
export function VisitorsPage({ onBack }: { onBack: () => void }) {
  ...
  const [form, setForm] = useState(empty), [invite, setInvite] = useState({ sponsorMemberId: '', scheduledDate: '' }), [history, setHistory] = useState<any>(null)
  ...
  const create = async (event: FormEvent) => {
    ...
    try { await apiRequest('/api/visitors', { method: 'POST', ...auth(), body: JSON.stringify(form) }); setShow(false); setForm(empty); setMessage('Visitante cadastrado.'); await load() }
  ...
  return <main className="community-page">
    ...
    {show && <Modal title="Novo visitante" onClose={() => !saving && setShow(false)}><form className="ui-form" onSubmit={create}><Feedback error={error} />
      ...
      <div className="form-actions"><button type="button" className="btn btn-secondary" disabled={saving} onClick={() => setShow(false)}>Cancelar</button><button className="btn btn-primary" disabled={saving}><Save size={16} /> {saving ? 'Salvando…' : 'Salvar'}</button></div>
    </form></Modal>}
