...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  app.get('/uploads/space-photos/:file', async (request:any, reply) => { try { const f=path.basename(request.params.file); const data=await fs.readFile(spacePhotoPath(f)); return reply.type(path.extname(f)==='.png'?'image/png':path.extname(f)==='.webp'?'image/webp':'image/jpeg').send(data) } catch { return reply.code(404).send({message:'Imagem não encontrada.'}) } })
